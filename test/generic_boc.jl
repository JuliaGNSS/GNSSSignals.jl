@testset "Generic BOC" begin

@testset "Generic BOC{$t, 0, $n}" for t in [GPSL1, GPSL5, GalileoE1B], n in [1, 5, 10]
    type = GenericBOC{t,0,n}
...
@testset "BOC($m,1) modulation" for m in [1,2,2.5,5,12.5]
    rate = 1e-3
    tau = 0:rate:10
    boc = get_code.(GenericBOC{GPSL1,m,1},tau,1)./get_code.(GPSL1,tau,1)
